...
@Service
class RideReadService(private val dsl: DSLContext) {

    fun listRides(page: Int, size: Int): RidePage {
        val offset = page * size

        val total = dsl.selectCount()
            .from(RIDE)
            .innerJoin(ACTIVITY).on(ACTIVITY.ID.eq(RIDE.ACTIVITY_ID))
            .fetchOne(0, Long::class.java) ?: 0L

        val rows = dsl
            .select(
                RIDE.ID, ACTIVITY.NAME, ACTIVITY.START_TIME,
                RIDE.DATE, RIDE.DISTANCE, RIDE.DURATION,
                RIDE.ELEVATION_GAIN, RIDE.AVG_POWER, RIDE.NORMALIZED_POWER,
                RIDE.TSS, RIDE.INTENSITY_FACTOR,
            )
            .from(RIDE)
            .innerJoin(ACTIVITY).on(ACTIVITY.ID.eq(RIDE.ACTIVITY_ID))
            ...
            .map { r ->
                RideSummary(
                    ...
                    startTime = r[ACTIVITY.START_TIME]?.let { parseStartTime(it) },
    ...
    fun getRide(rideId: Long): RideDetail? =
        dsl
            .select(
                RIDE.ID, ACTIVITY.EXTERNAL_ID, ACTIVITY.NAME, ACTIVITY.START_TIME,
                RIDE.DISTANCE, RIDE.ELEVATION_GAIN, RIDE.ELEVATION_DESCENT,
                RIDE.DURATION, RIDE.AVG_POWER, RIDE.MAX_POWER,
                RIDE.AVG_HR, RIDE.MAX_HR, RIDE.AVG_CADENCE, RIDE.MAX_CADENCE,
                RIDE.NORMALIZED_POWER, RIDE.INTENSITY_FACTOR, RIDE.TSS,
                RIDE.FTP, RIDE.WATTS_PER_KG,
                RIDE.BEST_POWER_5S, RIDE.BEST_POWER_30S, RIDE.BEST_POWER_1MIN,
                RIDE.BEST_POWER_5MIN, RIDE.BEST_POWER_10MIN,
                RIDE.BEST_POWER_20MIN, RIDE.BEST_POWER_60MIN,
                RIDE.RPE, RIDE.COACH_SUMMARY, RIDE.NOTES,
            )
            .from(RIDE)
            .innerJoin(ACTIVITY).on(ACTIVITY.ID.eq(RIDE.ACTIVITY_ID))
            ...
            .fetchOne { r ->
                RideDetail(
                    id = r[RIDE.ID]?.toLong(),
                    externalId = r[ACTIVITY.EXTERNAL_ID],
                    name = r[ACTIVITY.NAME],
                    startTime = r[ACTIVITY.START_TIME]?.let { parseStartTime(it) },
                    metrics = RideMetrics(
                        distanceKm = r[RIDE.DISTANCE]?.toDouble()?.let { it / 1000.0 },
    ...
    private fun parseStartTime(raw: String): OffsetDateTime? = runCatching {
        listOf(
            { OffsetDateTime.parse(raw) },
            { java.time.LocalDateTime.parse(raw, java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")).atOffset(ZoneOffset.UTC) },
            { java.time.LocalDateTime.parse(raw, java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")).atOffset(ZoneOffset.UTC) },
            { java.time.LocalDateTime.parse(raw, java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'")).atOffset(ZoneOffset.UTC) },
