...
class FtpTestDetectionService(
    ...
    fun onRideCalculated(event: RideCalculatedEvent) {
        ...
        log.info("FTP test rideId={} type={} ftpValue={}W{}", event.rideId, testType, validatedFtp.roundToInt(), if (notes != null) " [$notes]" else "")
    ...
    internal fun classifyTestType(name: String, metrics: RideMetrics): FtpTestType {
        val lower = name.lowercase()
        ...
        return when (nameType) {
            FtpTestType.RAMP_TEST -> if (profileMatchesRamp(metrics)) FtpTestType.RAMP_TEST else fallback(metrics)
            FtpTestType.SIXTY_MIN_TEST -> if (profileMatchesSixtyMin(metrics)) FtpTestType.SIXTY_MIN_TEST else fallback(metrics)
            FtpTestType.TWENTY_MIN_TEST -> if (profileMatchesTwentyMin(metrics)) FtpTestType.TWENTY_MIN_TEST else fallback(metrics)
            null -> fallback(metrics)
            else -> nameType
        }
    }
    ...
    private fun profileMatchesRamp(m: RideMetrics): Boolean {
        var score = 0
        val vi = m.variabilityIndex; val dur = m.durationSeconds
        val b1 = m.bestPower1min; val avg = m.avgPower; val b5 = m.bestPower5min; val b20 = m.bestPower20min
    ...
    private fun profileMatchesTwentyMin(m: RideMetrics): Boolean {
        var score = 0
        val vi = m.variabilityIndex; val dur = m.durationSeconds
        val b20 = m.bestPower20min; val avg = m.avgPower; val b60 = m.bestPower60min
    ...
    private fun profileMatchesSixtyMin(m: RideMetrics): Boolean {
        val dur = m.durationSeconds; val vi = m.variabilityIndex
        val avg = m.avgPower; val b60 = m.bestPower60min
    ...
    internal fun calculateFtp(testType: FtpTestType, metrics: RideMetrics): Double? = when (testType) {
        FtpTestType.RAMP_TEST -> metrics.bestPower1min?.takeIf { it > 0 }?.let { (it * 0.75).roundToInt().toDouble() }
        FtpTestType.TWENTY_MIN_TEST -> metrics.bestPower20min?.takeIf { it > 0 }?.let { (it * 0.95).roundToInt().toDouble() }
        FtpTestType.SIXTY_MIN_TEST -> metrics.bestPower60min?.takeIf { it > 0 }
        FtpTestType.UNKNOWN, FtpTestType.ESTIMATED -> null
    }
     ...
     */
    internal fun validate(ftp: Double, metrics: RideMetrics, previousFtp: Double?): Pair<Double?, String?> {
        // Absolute bounds
        ...
        val consistencyNote = buildString {
            val b5 = metrics.bestPower5min; val b10 = metrics.bestPower10min; val b60 = metrics.bestPower60min
        ...
        if (previousFtp != null && previousFtp > 0) {
            ...
            if (abs(deltaPct) > 15) {
                ...
                return Pair(ftp, buildNote("NEEDS_REVIEW: $severity $direction ${"%+.1f".format(deltaPct)}%", consistencyNote))
            }
        }
    ...
    private fun buildNote(primary: String, secondary: String?): String =
        if (secondary != null) "$primary; $secondary" else primary
}
