...
@Repository
class GarminActivitySyncCursorRepository(private val dsl: DSLContext) {

    fun findSince(): LocalDate? =
        dsl.select(GARMIN_ACTIVITY_SYNC_CURSOR.SINCE)
    ...
    fun updateSince(date: LocalDate) {
        dsl.insertInto(GARMIN_ACTIVITY_SYNC_CURSOR)
